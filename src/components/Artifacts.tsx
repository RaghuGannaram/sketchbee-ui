...
const Artifacts: React.FC = () => {
	...
	const scry = useStylus((state) => state.scry);
	...
	useEffect(() => {
		if (rite !== Rites.MANIFESTATION || casterSignature !== seerId) return;

		const handleChamberSync = () => {
			const vision = scry();

			emit("rune:shift", { chamberId, casterId: seerId, vision });
		};

		const unsubscribe = subscribe("chamber:sync", handleChamberSync);

		return () => {
			unsubscribe();
		};
	}, [rite, chamberId, seerId, casterSignature]);

	if (rite !== Rites.MANIFESTATION || casterSignature !== seerId) {
		return (
			<div className="w-full flex justify-center items-center p-4">
				<div className="flex items-center gap-3 px-6 py-2 bg-white/40 backdrop-blur-md rounded-full border border-indigo-100/50 shadow-sm">
					...
					<span className="text-[11px] font-mono font-bold text-indigo-900/40 tracking-widest">You'll get the controls when it's your turn</span>
				</div>
			</div>
		);
	}
