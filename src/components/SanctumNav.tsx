...
import { ArrowLeft, Clock, Users, Flame, Feather, Sun, Skull, HelpCircle, Scroll, User2, Sparkles, WandSparkles } from "lucide-react";
...
const SanctumNav: React.FC<ISanctumNavProps> = ({ rite, secondsLeft, enigma, epithet, onLeave }) => {
    ...
    const isCulminating = 0 < secondsLeft && secondsLeft <= 10;
                ...
                <div className={`flex justify-between items-center gap-0.5 pl-3 pr-5 py-2 ${isCulminating ? "text-red-400" : "text-stone-200"}`}>
                    {secondsLeft > 0 ? (
                        <>
                            ...
                            <span className={`min-w-7 font-mono text-base text-end font-bold leading-none tabular-nums `}>{secondsLeft}</span>
