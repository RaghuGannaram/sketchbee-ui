...
const SanctumNav: React.FC<ISanctumNavProps> = ({ rite, secondsLeft, onLeave }) => {
	...
	return (
		<nav className="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-y-4 md:gap-0">
			<div className="order-1 flex-1">
				...
				>
					...
					<span className="hidden md:block font-mono font-bold text-[10px] uppercase tracking-widest">Exit Room</span>
				</button>
			</div>
			...
			<div className="order-2 md:order-3 flex-1 flex justify-end items-center gap-10">
				{enigma && (
					<div className="relative group flex items-center gap-3 px-4 py-1 rounded-full bg-indigo-100 border border-indigo-400 shadow-lg shadow-indigo-900/20">
						<WandSparkles className="w-3 h-3 text-indigo-700" />
						<span className="font-mono font-black text-xs text-indigo-900 tracking-wide leading-none">{enigma.toUpperCase()}</span>
						<Sparkles className="w-3 h-3 text-indigo-500 absolute -top-1.5 -right-1.5 animate-pulse" />
					</div>
				)}
				{!enigma && omen && (
