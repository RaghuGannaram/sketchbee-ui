...
const Sanctum: React.FC = () => {
	...
	return (
		<div className="h-screen w-full flex flex-col justify-between px-2 md:px-8 py-2 md:py-4 gap-5 bg-linear-to-br from-slate-50 via-indigo-50/30 to-slate-100 font-serif">
			<SanctumNav rite={rite} secondsLeft={secondsLeft} onLeave={handleChamberLeave} />

			<div className="flex-1 grid grid-cols-1 xl:grid-cols-[320px_1fr_320px] gap-6">
				<div className="order-2 xl:order-1 h-[400px] xl:h-full">
					<SeerCircle />
				</div>

				<div className="order-1 xl:order-2 h-[400px] xl:h-full flex flex-col bg-white/40 backdrop-blur-md rounded-[2.5rem] border border-indigo-100 shadow-xl shadow-indigo-500/5 overflow-hidden">
					<Vellum />
				</div>

				<div className="order-3 xl:order-3 h-[400px] xl:h-[calc(100vh-11rem)]">
					<Whispers />
				</div>
			</div>
