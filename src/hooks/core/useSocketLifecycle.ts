...
function useConnection() {
	useEffect(() => {
		const handleConnect = () => {
			console.info("sketchbee-info: socket %s connected via %s: ", socketInstance.id, socketInstance.io.engine?.transport.name);
		};
	...
	const emit = useCallback((eventName: string, data: any, ackCallback?: (response: any) => void): Promise<any> | void => {
